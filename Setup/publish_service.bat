@echo off
echo Gerando publicação do SERVICO, aguarde...

:: Caminho do arquivo .csproj
set "filePath=%~dp0..\BasicService\BasicService.csproj"

:: Nome do arquivo temporário para redirecionar a saída do Inno Setup
set TEMP_LOG_FILE=%TEMP%\publish_log.txt
set PUBLISH_COMMAND=dotnet publish -f net8.0 -r win-x64 --self-contained true -o "C:\BasicApi" "%filePath%" --property:PublishSingleFile=true "%"filePath%"
